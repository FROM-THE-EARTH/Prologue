...
void Solver::update() {
    ...
    const double fixedtime =
        rocketAtDetached_.size() == 0 ? 0.0 : rocketAtDetached_[rocketAtDetached_.size() - 1].elapsedTime;
...
void Solver::updateParachuteStatus() {
    const bool detectpeakConditon = result_.rocket[targetRocketIndex_].maxHeight
                                    > rocket_.pos.z + AppSetting::GetSetting().simulation.detectPeakThreshold;
    ...
    const bool fixedtimeCondition =
        rocket_.elapsedTime > rocketSpec_.rocketParam[targetRocketIndex_].parachute[0].openingTime;
    ...
    const bool time_from_detect_peakCondition = rocket_.elapsedTime - result_.rocket[targetRocketIndex_].detectPeakTime
                                                > rocketSpec_.rocketParam[targetRocketIndex_].parachute[0].openingTime;
...
void Solver::updateParameters() {
    ...
    if (combustionTime_ <= rocketSpec_.rocketParam[targetRocketIndex_].engine.combustionTime()) {
        rocketDelta_.mass = (rocketSpec_.rocketParam[targetRocketIndex_].massFinal
                             - rocketSpec_.rocketParam[targetRocketIndex_].massInitial)
                            / rocketSpec_.rocketParam[targetRocketIndex_].engine.combustionTime();
        rocketDelta_.reflLength = (rocketSpec_.rocketParam[targetRocketIndex_].CGLengthFinal
                                   - rocketSpec_.rocketParam[targetRocketIndex_].CGLengthInitial)
                                  / rocketSpec_.rocketParam[targetRocketIndex_].engine.combustionTime();
        rocketDelta_.iyz = (rocketSpec_.rocketParam[targetRocketIndex_].rollingMomentInertiaFinal
                            - rocketSpec_.rocketParam[targetRocketIndex_].rollingMomentInertiaInitial)
                           / rocketSpec_.rocketParam[targetRocketIndex_].engine.combustionTime();
...
void Solver::calcDynamicForce() {
    ...
    if (!rocket_.parachuteOpened) {
        // Aero
        const double preForceCalc = 0.5 * air_->density() * rocket_.airSpeed_b.length() * rocket_.airSpeed_b.length()
        ...
        const double cd = rocketSpec_.rocketParam[targetRocketIndex_].airspeedParam.getParam().Cd
                          + rocketSpec_.rocketParam[targetRocketIndex_].airspeedParam.getParam().Cd_a2
                                * rocket_.attackAngle * rocket_.attackAngle;
        force_b_.x -= cd * preForceCalc * cos(rocket_.attackAngle);
        force_b_.y -= rocket_.Cny * preForceCalc;
        force_b_.z -= rocket_.Cnp * preForceCalc;

        // Moment
        const double preMomentCalc = 0.25 * air_->density() * rocket_.airSpeed_b.length()
        ...
        const double cp =
            rocketSpec_.rocketParam[targetRocketIndex_].airspeedParam.getParam().Cp
    ...
    if (rocket_.pos.length() <= rocketSpec_.env.railLength && rocket_.velocity.z >= 0.0) {  // launch
        ...
        const double drag        = 0.5 * air_->density() * paraSpeed.z * paraSpeed.z * 1.0
                            * rocketSpec_.rocketParam[targetRocketIndex_].parachute[rocket_.parachuteIndex].Cd;
